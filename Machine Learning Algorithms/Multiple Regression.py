...
for f in xlsx_files:
    ...
    mlr.fit(XTrainScaled,yTrainScaled)

    
    #Observed value versus predicted value
    
    yEstTrain = yScaler.inverse_transform(mlr.predict(XTrainScaled).reshape(-1, 1)).ravel()
    yEstTest = yScaler.inverse_transform(mlr.predict(XTestScaled).reshape(-1, 1)).ravel()
    ...
    writer = pd.ExcelWriter(os.path.basename(f)+'train'+'.xlsx')
    results_train.to_excel(writer,'results_train')
    writer.save()
    
    writer = pd.ExcelWriter(os.path.basename(f)+'test'+'.xlsx')

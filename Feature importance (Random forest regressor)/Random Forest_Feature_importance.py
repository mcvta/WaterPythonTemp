...
for f in xlsx_files:
    ...
    #Fitting the model
    rf = RandomForestRegressor(n_estimators = 50, max_depth = 485, min_samples_split = 5, max_features = 'auto' , bootstrap = True, random_state = 42)
    rf.fit(XTrainScaled,yTrainScaled)
    
    importance = pd.DataFrame(rf.feature_importances_)
    importance_final.append(importance)
    ...
    writer = pd.ExcelWriter('1-'+os.path.basename(f)+'train_importance'+'.xlsx')
    importance.to_excel(writer,'importance')
    writer.save()
...
importance_df = pd.concat(importance_final)
importance_F = pd.DataFrame(importance_df)    
importance_F.to_csv('G:\My Drive\Python_Projects\ANN WQ model input data\importance.csv')    
